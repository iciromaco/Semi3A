...
class MyWidget(BoxLayout):
    ...
    def orb(self):
        ...
        srcimg = cv2.cvtColor(self.gryimg,cv2.COLOR_GRAY2BGR)
    ...
    def do_menu(self):
        ...
        if self.mode == 'ORB特徴':
            self.ids['pic1'].texture = self.orb()
        if self.mode == '開く':
            self.show_load()
        if self.mode == '保存':
            self.show_save()
    ...
    def imread(self,filepath):
        imgarray = np.fromfile(filepath, dtype=np.uint8)
        return cv2.imdecode(imgarray,cv2.IMREAD_COLOR)
...
class MyApp(App):
    def build(self):
        ...
        mywidget.ids['sp0'].values = ('開く','２階調化',"ORB特徴",'保存')
